...
namespace MoreMountains.CorgiEngine
{
    ...
    public class Weapon : MMMonoBehaviour
    {
        ...
        {
            float coolTime = Time.time - _lastTurnWeaponOnAt;

            if (coolTime < TimeBetweenUses && !isCooldown)
            {
                isCooldown = true; // 쿨다운 플래그 설정
                StartCoroutine(CooldownWeapon(TimeBetweenUses - coolTime));
            }
            else if (!isCooldown)
            {
                UseWeapon();
        ...
        private IEnumerator CooldownWeapon(float coolTime)
        {
            yield return new WaitForSeconds(coolTime);
            isCooldown = false; // 쿨다운 플래그 해제
            UseWeapon();
        }
        //**************수정한 코드**************
